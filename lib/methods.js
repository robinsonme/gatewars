Meteor.methods({
  ...
  'purchaseGrowth': function(number=1) {
    ...
    console.log("Cost in method: " + cost);
  ...
  'resetGrowthRate': function() {
    var currentUser = Meteor.user();
    Players.update({createdBy: currentUser._id}, {$set:{"growth": 1, growthCost: 100 }});
  },
  'killCitizens': function() {
    var currentUser = Meteor.user();
    Players.update({createdBy: currentUser._id}, {$set:{"citizens": 0 }});
  },
  'killWorkers': function() {
    ...
    for (var i = 0; i < workers.length; i++) {
      ...
      var selector = {};
      selector[name] = 0;
      Players.update({createdBy: currentUser._id}, {$set: selector});
      Players.update({createdBy: currentUser._id}, {$set:{'rate': Meteor.call("calculateRate")}})
    }
  },
});
